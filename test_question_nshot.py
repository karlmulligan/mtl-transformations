...
obs_list = []
obs_cols = ["modelname", "task", "side_task", "side_task_strat", "n_shot", "modeltype", "attention", "learning_rate", "hidden_size", "random_seed", \
        "which_set", "input", "target", "pred", "full_right", "first_aux", "other_aux"]
#        "full_right", "full_except_period", "truncated_right", "entire_np_order", "main_dp_order", "main_dp_patient_only", "agreement", \
#        "subj_rc", "obj_rc", "subj_pp", "obj_pp", "intransitive", "token_passive", "simple_patient", "patient_distractor"]
...
while direcs_to_process:
        if not os.path.exists(directory + "_" +  str(counter)):
                ...
                mainfix = "_".join(sys.argv[2].split("_")[:-1])

                # EVALUATE ON TEST SET
                basic_ob = {"modelname" : directory.split("/")[1],
                            "task": mainfix.split("_")[0],
                            "side_task": mainfix.split("_")[1] if mainfix.find("_") != -1 else "none",
                            "side_task_strat": "_".join(mainfix.split("_")[1:]) if mainfix.find("_") != -1 else "none",
                            "n_shot": sys.argv[2].split("_")[-1].split("-")[0],
                ...
                for this_batch in test_batches:
                        ...
                        for trio in zip(input_sents, target_sents, pred_sents):
                            ...
                            #this_ob['subj_rc'] = subj_rc(i.split())
                            #this_ob['obj_rc'] = obj_rc(i.split())
                            #this_ob['subj_pp'] = subj_pp(i.split())
                            #this_ob['obj_pp'] = obj_pp(i.split())
                            #this_ob['intransitive'] = intransitive(i.split())
                            #this_ob['token_passive'] = token_passive(i.split())
                            #this_ob['simple_patient'] = simple_patient(t.split())
                            #this_ob['patient_distractor'] = patient_distractor(t.split())
                            # evaluation metrics
                            this_ob['full_right'] = full_right(i, t, p)
                            this_ob['first_aux'] = 1 if t.split()[0] == p.split()[0] else 0
                            this_ob['other_aux'] = 1 if p.split()[0] in auxes else 0
                            #this_ob['full_except_period'] = full_except_period(i, t, p)
                            #this_ob['truncated_right'] = truncated_right(i, t, p)
                            #this_ob['entire_np_order'] = entire_np_order(i, t, p)
                            #this_ob['main_dp_order'] = main_dp_order(i, t, p)
                            #this_ob['main_dp_patient_only'] = main_dp_patient_only(i, t, p)
                            #this_ob['agreement'] = agreement(i, t, p)
...
results_csv = "results/obs_results_question_nshot.csv".format(directory.split("/")[1])
